...
class CompassThread(threading.Thread):
    ...
    def run(self):
        ...
        p = subprocess.Popen(['compass', 'compile', self.dirname],
...
class CompassPreprocessor(LiveReload.Plugin,
    ...
    description = 'Compile and refresh'
    file_types = '.scss'

    def on_post_save(self, view):
        if view.file_name().find('.scss') > 0 \
            and bool(self.settings.get('compass_css_dir')):
            filename = view.file_name().replace('sass',
                    self.settings.get('compass_css_dir'
                    )).replace('.scss', '.css')
